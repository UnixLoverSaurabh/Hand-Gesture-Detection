...
class DataLoader:
    ...
    def load_data(self, path):
        ...
        if os.path.exists(path + self.raw_data_filename):
            with open(path + self.raw_data_filename) as complete_file:
    ...
    def load_image(self, image_path):
        ...
        # nested for loop for visual appeal, running (rex_x * rex_y) number of times
        raw_count = 0
        ...
        raw_RGB = np.reshape(raw_RGB, (50, 50, 1))
        return raw_RGB
    ...
    def get_set_elements_to_train(self, index):
        ...
        print("Shape of input_images_set[0] ", np.shape(self.input_images_set[index]))
        print("Shape of input_reals_set[0] ", np.shape(self.input_reals_set[index]))
        print("Shape of output_reals_set[0] ", np.shape(self.output_reals_set[index]))
    ...
    def combine_data(self, random_sort=False):
        ...
        for i in range(len(self.input_images_set)):
            for j in range(len(self.input_images_set[i])):
        ...
        print("Input_images_set before combine_data ", np.shape(self.input_images_set))
        ...
        if not random_sort:
            ...
            for i in range(len(new_input_images_set)):
        ...
        print("Input_images_set after combine_data ", np.shape(self.input_images_set))
