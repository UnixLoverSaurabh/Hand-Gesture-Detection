"""
NetLoader.py
Deals with loading models file, training and prediction.
NetLoader is using a library called Keras which MUST BE installed in order for
...
class NetLoader:
    ...
    def __init__(self, model_file="", weights_file="", learning_rate=0.01,
        ...
        if not model_file == "":
            ...
            if (not self.copy_path == "") and self.copy:
    ...
    '''
    Compile model with an optimizer which will used for training
    '''
    ...
    def predict(self, inputs):
        prediction = self.model.predict(x=inputs, batch_size=1, verbose=2)
        return prediction
    ...
    def fit(self, inputs, outputs, epochs=1, verbose=0):
        ...
        self.model.fit(x=inputs, y=outputs, batch_size=1, epochs=epochs, verbose=verbose)
